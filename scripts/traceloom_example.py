...
# 配置日志
logger.add(
    settings.LOG_DIR / "traceloom_example.log",
    rotation="10 MB",
    compression="zip",
...
def main():
    ...
    try:
        ...
        sample_file = settings.RAW_DATA_DIR / "20251203_230356_b6x-playback.txt"
        reweave_result = tl.reweave(
            input_file=str(sample_file),
            output=str(settings.OUTPUT_DIR / "reweave_path.txt")
        )
        ...
        embroider_result = tl.embroider(
            ...
            output=str(settings.OUTPUT_DIR / "embroider_path.txt")
        )
        ...
        dream_result = tl.dream(
            input_pattern="s0x2 -> s2x6",
            output=str(settings.OUTPUT_DIR / "dream_path.txt")
        )
        ...
        json_result = tl.embroider(
            ...
            output=str(settings.OUTPUT_DIR / "json_pattern_path.txt")
        )
