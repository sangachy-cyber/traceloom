...
def test_extract_16d_features():
    ...
    observations = []
    for _i in range(100):
        obs = Observation(delay_up=100.5, loss_up=0.01, bw_up=10.0, delay_down=100.5, loss_down=0.01, bw_down=10.0)
...
def test_extract_features_with_raw_profile():
    ...
    # 创建 RawProfile
    raw_profile = RawProfile(trace_name="test_trace", start_index=0, observations=observations, is_valid=True)
